...
const ModelViewer: React.FC<ModelViewerProps> = ({
  ...
  useEffect(() => {
    ...
    gltfLoader.load(
      ...
      (gltf) => {
        ...
        // <<< MODIFICATION START: Detect URL parameter and set initial texture >>>
        ...
        // 2. Decide which texture to load: the one from the URL or the default one.
        const initialTexturePath = imageUrlFromUrl || '/tex-default-preview.png';

        // 3. Load the determined texture.
        ...
        if (initialTextureLoader) {
          initialTextureLoader.load(
            ...
            (texture) => {
              ...
              if (material) {
                // Configure and apply the texture (this works for both default and URL-based)
                texture.flipY = false;
  ...
  // Texture update hook (for file uploads) - UNCHANGED
  useEffect(() => {
